...
const BENTO_FEATURES = [
  ...
  },
  {
    ...
    desc: 'Kiến trúc 2 cấu hình: Edge Profile dùng model gọn và Studio Profile dùng YOLOv8x-pose CUDA FP16 cho 2-6 người.',
  ...
  },
  {
    ...
    desc: 'Tính toán khoảng cách Euclidean chuẩn hóa và ma trận trọng số giải phẫu học anatomy-weighted cosine bằng NumPy.',
...
export default function HomePage() {
  ...
  return (
    <main ref={mainRef} className="w-full max-w-full bg-background text-foreground min-h-screen">
      {/* Floating Glass Navigation */}
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 flex items-center justify-between backdrop-blur-2xl bg-background/85 border-b border-border">
        <Link href="/" className="flex items-center gap-2.5 text-decoration-none group">
          ...
          <span className="font-bold text-base tracking-tight text-foreground group-hover:text-primary transition">
            Pose-Booth <span className="text-violet-400">AI</span>
          </span>
        </Link>

        <div className="flex items-center gap-6">
          <Link href="/booth" className="text-xs font-mono text-muted-foreground hover:text-foreground transition hidden sm:block">
            PHOTOBOOTH
          </Link>
          <Link href="/pose-studio" className="text-xs font-mono text-muted-foreground hover:text-foreground transition hidden sm:block">
            POSE STUDIO
          </Link>
          <Link href="/about" className="text-xs font-mono text-muted-foreground hover:text-foreground transition hidden sm:block">
            ARCHITECTURE
          </Link>
          <Link
            href="/booth"
      ...
      <section className="pt-36 sm:pt-44 pb-20 px-6 max-w-7xl mx-auto text-center relative z-10">
        {/* Subtle Brand Pill */}
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-secondary border border-border text-xs font-mono text-muted-foreground mb-8">
          <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse" />
          <span>HUAWEI CAMERA AR CONTOUR + DUAL-PROFILE AI</span>
        </div>

        {/* 2-Line Ultra-Wide H1 */}
        <h1 className="hero-headline text-4xl sm:text-6xl md:text-7xl font-bold tracking-tight text-foreground max-w-5xl mx-auto leading-[1.08] mb-6">
        ...
        <p className="hero-subtext text-base sm:text-lg text-slate-400 max-w-2xl mx-auto leading-relaxed mb-10">
          Xóa bỏ que xương stickman thô kệch. Trải nghiệm đường viền lụa AR thanh mảnh và so khớp dáng thời gian thực trên card đồ họa RTX.
        </p>
        ...
        <div className="hero-actions flex flex-wrap items-center justify-center gap-4">
          ...
          <Link
            href="/pose-studio"
            className="px-7 py-3.5 rounded-xl bg-secondary hover:bg-accent border border-border text-secondary-foreground font-medium text-sm tracking-tight backdrop-blur-md transition"
          >
      ...
      <footer className="border-t border-border py-12 px-6 max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-mono text-muted-foreground">
        <div>
